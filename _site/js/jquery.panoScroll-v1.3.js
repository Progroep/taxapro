...
(function($){
 	$.fn.extend({ 
 		...
 		panoScroll: function(options) {
			...
			//code for pause
			$("#"+SN+" div.sLeft").mouseout(function(){ obj.stopTime(name); $(SN+" div.sCtrl").fadeIn(); });
			});

			//code for scrolling right
			$("#"+SN+" div.sRight").hover(function(){
				obj.everyTime(SS, name, function(n) {
					OP = OP-NP;
					obj.css("background-position",OP+"px 0");
					return OP;
			...
			//code for pause
			$("#"+SN+" div.sRight").mouseout(function(){ obj.stopTime(name); $(SN+" div.sCtrl").fadeIn(); });
			});
